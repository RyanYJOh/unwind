/// §7.2 Todd 인터페이스 — 최종 에셋(또는 Rive)으로 교체할 때도 이 인터페이스를
/// 유지한다. ToddView는 이 상태만 받아 렌더링한다.
/// [poke]: 사용자가 Todd를 톡 건드렸다 (개편 2026-08-12).
/// 반응은 렌더러가 **자기 모드를 보고** 고른다 — 깨어 있으면 간지럼,
/// 졸리면 실눈으로 두리번, 잠들었으면 무반응.
/// [wakeUp]: 기지개+미소로 깨어난다 (2026-08-22) — isAsleep 하강 에지가
/// 없는 국면(온보딩의 뒤척이는 중 기상)에서도 기상 연출을 쓰기 위한 명시
/// 이벤트.
enum ToddEvent { blink, yawn, react, fallAsleep, poke, wakeUp }

/// Todd의 하루 (개편 2026-08-08) — 시각·체크리스트 상태로 결정되는 생활 모드.
...
class ToddState {
  ...
  /// [ToddMode.day]일 때의 현재 일과
  final ToddDayActivity? activity;

  /// [ToddMode.nightAwake]일 때 눈부심 정도 (= 방에 남은 빛, 1 - t)
  final double dazzle;
