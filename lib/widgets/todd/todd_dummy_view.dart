...
import 'todd_parts.dart';

/// §7 Todd 뷰 (CustomPainter 더미 렌더러).
/// 렌더러 스위치(todd_view.dart)에서 Rive 실패 폴백 및 비교용으로 사용.
///
/// §7.3 행동 규칙 / §1.3 금지: 원망·실망 표현 없음. 졸림·기다림·안도만.
class ToddDummyView extends StatefulWidget {
  ...
  @override
  State<ToddDummyView> createState() => _ToddDummyViewState();
}

class _ToddDummyViewState extends State<ToddDummyView>
  ...
  void initState() {
    ...
    _react = AnimationController(
      vsync: this,
      duration: const Duration(milliseconds: UnwindMotion.toddReactMs),
    );
    ...
    _sleep = AnimationController(
      vsync: this,
      duration: const Duration(milliseconds: UnwindMotion.toddFallAsleepMs),
    );
    _scheduleBehavior();
  }

  @override
  void didUpdateWidget(ToddDummyView old) {
    ...
    if (widget.state.eventTick != _seenEventTick) {
      ...
      if (widget.state.event == ToddEvent.react && !widget.state.isAsleep) {
  ...
  void _scheduleBehavior() {
    ...
    if (b < 0.3) {
      ...
      next = Duration(
        milliseconds:
            (UnwindMotion.toddBlinkMinS * 1000 +
                    _rng.nextInt(
                      (UnwindMotion.toddBlinkMaxS -
                              UnwindMotion.toddBlinkMinS) *
                          1000,
                    ))
                .toInt(),
      );
      ...
      if (yawnNow) {
        next = Duration(
          milliseconds:
              UnwindMotion.toddYawnMinS * 1000 +
              _rng.nextInt(
                (UnwindMotion.toddYawnMaxS - UnwindMotion.toddYawnMinS) * 1000,
              ),
        );
        action = _doYawn;
      } else {
    ...
    return RepaintBoundary(
      child: AnimatedBuilder(
        ...
        builder: (context, _) {
          ...
          final reactScale =
              1.0 +
              math.sin(_react.value * math.pi) * UnwindMotion.toddReactScale;
