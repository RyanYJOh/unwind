...
class _GhostPainterViewState extends State<GhostPainterView>
  ...
  /// 사용자가 Todd를 톡 건드렸을 때 (개편 2026-08-12).
  ...
  /// 몸통 PNG (개편 2026-08-12, kToddBodyStyle.image) — 앱 전역 1회 로드.
  ...
  void _ensureBodyImage() {
    if (kToddBodyStyle != ToddBodyStyle.image || _bodyImage != null) return;
  ...
  double get _effSleepiness {
    ...
    return switch (widget.mode) {
      null => base,
      ToddMode.day => widget.activity == ToddDayActivity.rest ? 0.30 : 0.06,
      ToddMode.nightAwake => _lerp(0.68, 0.45, widget.dazzle.clamp(0.0, 1.0)),
      ToddMode.asleep => math.max(base, 0.85),
    };
  }
  ...
  Future<void> _firePoke() async {
    ...
    final peek = widget.mode == ToddMode.nightAwake || _effSleepiness >= 0.55;
        ...
        builder: (context, _) {
          ...
          return CustomPaint(
            ...
            painter: _GhostPainter(
              ...
              sleepiness: t,
              asleepProgress: s,
              bodyImage: kToddBodyStyle == ToddBodyStyle.image
                  ? _bodyImage
                  : null,
              // 생활 레이어 (개편 2026-08-08) — 잠들면 활동·밤 연출 없음
              activity: widget.mode == ToddMode.day && s < 0.5
                  ? widget.activity
                  : null,
              nightDoze: widget.mode == ToddMode.nightAwake && s < 0.5,
...
class _GhostPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    double leanDeg = 0;
    if (activity == ToddDayActivity.walk) {
      // 산책 (개정 2026-08-16): 예전 느린 순찰과 너무 들뜬 통통의 중간.
      final wp = phase * 2 * math.pi * 0.33;
      final step = math.sin(phase * 2 * math.pi * 1.7);
      final wanderX = math.sin(wp) * 28 * u + step * 3 * u;
      final bob = math.sin(phase * 2 * math.pi * 2.15).abs() * 5.5 * u;
      leanDeg = math.cos(wp) * 10.0 + step * 2.0;
      canvas.translate(wanderX, -bob);
    } else if (activity == ToddDayActivity.hum) {
      // 콧노래: 리듬 타는 좌우 스웨이
      leanDeg = math.sin(phase * 2 * math.pi * 0.5) * 2.4;
    } else if (activity == ToddDayActivity.dance) {
    ...
    if (activity == ToddDayActivity.stretch) {
      // 아침 스트레칭: 위로 쭉 — 세로 늘어남
    ...
    if (bodyImage != null) {
      // ── 몸통 PNG (개편 2026-08-12, kToddBodyStyle.image):
    ...
    final eyeYOff =
        (activity == ToddDayActivity.read ||
            activity == ToddDayActivity.doodle)
        ? 3.0 * u
        : 0.0;
    ...
    for (final dir in [-1, 1]) {
      ...
      // 2차: 빗금은 과해서 걷어냈다 — 음영만 남긴다).
      // 눈 모양 분기(간지럼·취침)보다 먼저 그린다 — 전날 못 잔 흔적은
      // 감은 눈 밑에도, 웃는 눈 밑에도 남아 있어야 한다 (세계관 2026-08-15).
      // **꽉 채운 음영 주머니**(짙은 슬레이트-퍼플)가 세기에 따라 넓고
      // 짙어지고, 아랫단 윤곽선이 처진 주머니를 잡아 준다.
      if (darkCircleOpacity > 0.01) {
        ...
        final bag = Path()
          ...
          ..close();
        // 채운 음영 — 항상 그린다 (옅어도 그늘이 보여야 "피곤"이 읽힌다)
        canvas.drawPath(
          bag,
          Paint()..color = const Color(0xFF8B7396).withValues(alpha: 0.55 * op),
        );
        // 아랫단 가장자리 — 주머니의 처진 윤곽
        final edge = Path()
          ..moveTo(ec.dx - dcW, topY)
    ...
    if (activity == ToddDayActivity.read && asleepProgress < 0.5) {
    ...
    final restSmile = activity == ToddDayActivity.rest && asleepProgress <= 0.6;
    if (mouthOpen > 0.08) {
    ...
    } else if (activity == ToddDayActivity.bubbles) {
      ...
      final chew = activity == ToddDayActivity.snack
    ...
    switch (activity) {
      case ToddDayActivity.read:
        _paintBook(canvas, u, cx, cy, bodyH);
      case ToddDayActivity.coffee:
        _paintCoffee(canvas, u, cx, cy, bodyH);
      case ToddDayActivity.snack:
        _paintCookie(canvas, u, cx, cy, bodyH);
      case ToddDayActivity.doodle:
        _paintDoodle(canvas, u, cx, cy, bodyH);
      case ToddDayActivity.dance:
    ...
    if (activity == ToddDayActivity.hum) {
      for (var i = 0; i < 2; i++) {
    ...
    if (activity == ToddDayActivity.bubbles) {
      for (var i = 0; i < 3; i++) {
