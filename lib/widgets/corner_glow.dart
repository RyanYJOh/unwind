...
class _CornerGlowPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // Lumi가 눈이 부셔 잠들 수 없어야 한다. 그래야 하나씩 끌 때마다 방이
    ...
    // 불투명 면에 얹는 것이 먼저다.
    // 1. 워시 — 방 전체로 스미는 온기
    glow(w * (1.05 + 1.45 * ease), _wash, 0.52 * ease);
    // 2. 미드 — 상부를 채우는 빛
    glow(w * (0.70 + 1.05 * ease), _mid, 0.78 * ease);
    // 3. 코어 — 코너의 빛덩어리
    glow(w * (0.38 + 0.60 * ease), _core, 1.0 * ease, focus: 0.10);
    // 4. 핫스팟 — 광원 자체의 눈부심 (거의 흰색)
    glow(w * (0.22 + 0.34 * ease), _hot, 1.0 * ease, focus: 0.22);
  }
