...
class BillDao extends DatabaseAccessor<UnwindDatabase> with _$BillDaoMixin {
  ...
  Future<WeeklyBill?> getBill(String weekStart) => (select(
    weeklyBills,
  )..where((b) => b.weekStart.equals(weekStart))).getSingleOrNull();

  /// [before] 이전의 가장 최근 청구서 (지난주 대비 문구용)
  Future<WeeklyBill?> getLatestBefore(String before) =>
      (select(weeklyBills)
  ...
  Future<void> markRead(String weekStart) =>
      (update(weeklyBills)..where((b) => b.weekStart.equals(weekStart))).write(
        const WeeklyBillsCompanion(isRead: Value(true)),
      );
}
