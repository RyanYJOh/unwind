...
//
// - 데이터: Flutter 앱이 App Group UserDefaults에 쓰는 스냅샷
//   (lib/domain/services/widget_snapshot_service.dart 와 키·의미가 계약).
// - Lumi: 앱 페인터로 사전 렌더한 스프라이트 PNG (재추출:
//   SPRITE_EXPORT=1 flutter test test/tools/widget_sprite_export_test.dart).
// - 모드 판정은 lumiModeProvider(§4)의 미러 — 앱 쪽 규칙이 바뀌면 여기도
//   맞춰야 한다.
...
private struct Snapshot {
    ...
    static func load() -> Snapshot? {
        guard let d = UserDefaults(suiteName: appGroupId),
              let dayKey = d.string(forKey: "dayKey")
        else { return nil }
        return Snapshot(
            dayKey: dayKey,
            remaining: d.integer(forKey: "remaining"),
            total: d.integer(forKey: "total"),
            ...
            wakeHour: d.object(forKey: "wakeHour") as? Int ?? 5,
            bedtimeHour: d.object(forKey: "bedtimeHour") as? Int ?? 22,
            languageCode: d.string(forKey: "languageCode") ?? "en"
        )
    }
}

// MARK: - 장면 판정 (lumiModeProvider §4 미러)
...
private func computeState(at date: Date, snapshot: Snapshot?) -> DisplayState {
    ...
    // Dart lumiModeProvider와 동일 (개정 2026-08-15): 기상~취침을 활동
...
// Dart LumiDayActivity enum 순서와 반드시 일치할 것 (개정 2026-08-15: +3종)
...
private extension DisplayState {
    var spriteName: String {
        let base: String
        switch scene {
        case .asleep: base = "lumi_asleep"
        case .day(let slot), .morning(let slot): base = "lumi_day_\(daySlotNames[slot])"
        case .nightSquint: base = "lumi_night_squint"
        case .nightDoze: base = "lumi_night_doze"
        }
    ...
    var pillLabel: String {
        ...
        case .asleep(let satisfied):
            if satisfied { return ko ? "다 껐어요" : "All off" }
            return ko ? "새근새근" : "Fast asleep"
        case .morning: return ko ? "좋은 아침!" : "Good morning!"
        case .day: return ko ? "조용한 방" : "A quiet room"
...
private struct LumiEntry: TimelineEntry {
    let date: Date
    let state: DisplayState
}

private struct LumiProvider: TimelineProvider {
    ...
    func getSnapshot(in context: Context, completion: @escaping (LumiEntry) -> Void) {
        if context.isPreview {
            completion(placeholder(in: context))
        } else {
            completion(LumiEntry(date: .now, state: computeState(at: .now, snapshot: Snapshot.load())))
        }
    }

    func getTimeline(in context: Context, completion: @escaping (Timeline<LumiEntry>) -> Void) {
        // 매시 정각 엔트리 24개 — 일과 슬롯(2h)·취침·기상 전환을 모두 덮는다.
        // 앱이 상태를 바꾸면 updateWidget이 타임라인을 통째로 다시 뽑는다.
        let snapshot = Snapshot.load()
        let now = Date()
        var entries = [LumiEntry(date: now, state: computeState(at: now, snapshot: snapshot))]
        ...
        if let nextHour = cal.nextDate(
            ...
            for i in 0..<24 {
                ...
                entries.append(LumiEntry(date: date, state: computeState(at: date, snapshot: snapshot)))
            }
        }
...
private struct LumiWidgetView: View {
    let entry: LumiEntry
...
struct LumiWidget: Widget {
    ...
    var body: some WidgetConfiguration {
        StaticConfiguration(kind: kind, provider: LumiProvider()) { entry in
            LumiWidgetView(entry: entry)
        }
        .configurationDisplayName("Lumi")
        .description("오늘 남은 불빛과 Lumi의 하루 · Lumi's day and the lights still on")
