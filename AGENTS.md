...
- **다크서클 (세계관 2026-08-15)**: 전날 밤 불을 남긴 채 넘어왔다면
  ...
  얹는다. **리얼 개정 2026-08-22 (발주자 지시, 2차: 빗금 제거)**: 선 하나로는
  약했다 — **꽉 채운 음영 주머니**(짙은 슬레이트-퍼플 #8B7396 채움 +
  #6E5480 아랫단 윤곽)로 그린다. 빗금은 과해서 걷어냈다 (음영만).
  세기가 높을수록 넓고 짙어진다. **외형이
...
1. 상단 (개편 2026-08-13): **좌측 끝 청구서 버튼** — 지난주 영수증을
   **요일 무관** 연다 (정책 개정 2026-08-28, 발주자 지시 — 월요일 게이트·
   잠금 안내 영수증 폐지). 미확인 코랄 점도 요일 무관, 지난주 영수증을
   읽을 때까지 (옆의 "Bill" 칩에서 점으로. 앰버는 앱 전체가 쓰는 색이라
   알림으로 안 읽힌다) + 날짜 타이틀 ("Today" / 과거면 "Aug 7") +
   **작은 설정 아이콘**(제목 오른쪽, 32pt)
   **스크롤 (개정 2026-09-14, 발주자 지시)**: 1~3(헤더·Todd·체크리스트)이
   `CustomScrollView` 하나로 **함께** 스크롤된다 — 이전엔 리스트만 스크롤.
   고정은 하단 4(주 칩·FAB·스트립)와 6(전등 줄 — 스크롤 밖 Stack 형제),
   배경(글로우·별)뿐이다. 빈 방 문구는 `SliverFillRemaining`으로 Todd 아래
   남은 화면 가운데. 스크롤된 타일이 손잡이 밑을 지나면 그 자리 탭은 줄이
   받는다(painter.hitTest 범위) — 조금 더 스크롤하면 된다.
...
4. 하단 (개편 2026-08-13) — **주 칩 + 주간 스트립**이 너비를 다 쓴다
   ...
   - **스트립은 주 단위 페이징**이다: 한 페이지 = 월~일 한 주, 가로로 넘기면
     ...
     **요일**(Mon·Tue…). **앰버로 밝은 창은 오늘 하나뿐이다** (개정
     ...
     음영 주머니 문법, 리얼 개정 2026-08-22), 다 끄고 잔 밤이면 스마일.
...
- **청구서** (§6.5, 정책 개정 2026-08-28): 지난주(월~일) 영수증을
  **요일 무관** 연다 — 월요일 게이트(2026-08-16)와 잠금 안내 영수증은
  폐지했다. 생성은 여전히 주 1회(롤오버가 지난주 몫을 만든다), 발송
  알림은 월요일 09:00 그대로. 영수증은 세 구역 —
  ① Todd 상태(카드, 이미지+문구 한 줄) ② 완수한 일 `n / m`(가장 낮음)
  ③ 전기요금(요일별 합산, 메인 — 요일 행의 금액 앞에 그날 밤 **남긴 등
  개수**를 옅은 캡션으로 단다: "등 2개" / "2 lights". 고지서의 "사용량 ·
  금액" 문법. 닫은 날은 점만. 2차 개정 2026-08-28 — ×N은 전달이 약했다).
...
- **알림** (§10): 네 종류. 설정 > **푸시**에서 각각 on/off.
  ① 아침 인사(기상시간 +1h, 매일 반복 — 문구 개정 2026-08-23: **그날 할 일이
  남아 있으면 개수로 응원**한다 "Rise and shine! Let's smash {n} tasks today",
  0개면 기존 인사 그대로. 본문은 예약하는 순간 굳으므로
  `morningGreetingSchedulerProvider`가 **발송될 날의 방**을 세어 개수가
  바뀔 때마다 재예약한다 — 반복은 14일치가 미리 전개돼 있어 내일 몫도
  이미 DB에 있지만, 자동 미루기로 넘어올 잔여분은 롤오버 전이라 못 센다) ② 하루 마무리/Unwind(취침
  ...
  0.5초 후.
  **소리·진동 (개정 2026-09-14, 발주자 지시 — 기존 무음 원칙 폐기)**:
  모든 알림(넛지 포함)이 기본 알림음과 진동을 낸다. 발송 방식은
  `NotificationService._details` 한 곳에만 둔다. ⚠️ iOS 플러그인은
  `presentSound: true`일 때만 `content.sound`를 채운다 — false면 백그라운드
  에서도 무음이고, iOS 진동은 소리를 따라가므로 진동도 사라진다(진동 여부는
  기기 햅틱 설정). 이미 예약된 옛 무음 알림은 앱을 열 때 스케줄러가 같은
  id로 재예약하며 교체된다.
...
5. 8. **원형 타임테이블** — 24시간 링(자정이 위), 수면 호(슬레이트+달)·
   ...
   **심사 빌드에선 별점 팝업을 뺀다** — `lib/core/build_flags.dart`의
   ...
   그대로이고 팝업만 건너뛴다.
   ⚠️ 심사 통과 바이너리가 곧 출시 바이너리다 — `true`인 채로 출시하면
   유저에게도 팝업이 안 뜬다.
...
- **데이터**: 로컬 온리라 서버가 필요 없다. `widgetSyncProvider`(providers.dart,
  ...
  **write는 180ms 디바운스** (2026-08-22): 체크 하나에도 todos·days 두
  ...
  고정되던 버그, 2026-08-16).
  **+ resume 플러시 & 자가 회복** (2026-08-29 — "다음날 Good morning 고착"
  마지막 구멍 세 개): ① 롤오버가 이미 끝난 날의 조용한 resume은 어떤
  ...
  **리로드를 늘리는 수정은 다시 하지 말 것.**
  계약은 `test/domain/services/widget_snapshot_service_test.dart`가 지킨다.
  **+ 생성 기록 계측** (2026-09-14, 1.0.1+35 — 대책 후에도 "Todd가 졸린
  시간대 진입 뒤엔 체크·추가·삭제가 위젯에 안 실리고 다음날엔 된다"는
  보고): 앱·위젯 코드엔 시각 게이트가 없고(두 번 검토, iOS 26.5 시뮬레이터
  재현 불가), Apple 엔지니어 답변상 리로드 수락은 시스템 재량(앱 사용
  빈도·배터리·리로드 빈도)이다. 판정을 위해 `ToddWidget.getTimeline`이
  생성 시각과 그때 읽은 개수를 App Group `widget_lastgen.json`에 남기고
  (`recordGeneration`), 설정 > Widget diagnostics(dev, 버전 10탭)가
  **플러시 전** 상태를 먼저 읽어 보여준다: `write(before)`·`body(before)`
  (앱이 쓴 것) vs `widget lastGen`(WidgetKit이 실제로 재생성한 것).
  판정표 — body는 새것인데 lastGen이 옛것: WidgetKit이 리로드를 안 받아줌
...
- **게이트 ③ 위젯 배경** (2026-08-28, 3차: 별도 갤러리 화면 폐기):
  ...
  설정 최상단 Plus 배너·조명 색과 함께 **항상 노출** (2026-09-17 — 이전엔
  심사 빌드에서 숨김). 버전 10탭 잠금은 이제 "데이터" 섹션(데이터 삭제·
  dev 도구)에만 남아 있다.
