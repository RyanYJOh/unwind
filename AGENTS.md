...
flutter analyze && flutter test                           # 58개 통과가 기준선
flutter run                                               # 개발 실행
...
2. **day** (dayStartHour~19시) — 행복한 펫. 2시간 슬롯 일과
   ...
   레이어 — 외형을 바꿔도 소품 코드는 유지된다. 잔·쿠키는 손(`_paintHoldingHand`)
   이 쥐고, 독서는 안경 + 양손 받침.
3. **nightAwake** (19시~, 미완 항목 존재) — 못 자는 밤. `dazzle = 1 - t`:
   빛이 많이 남았으면(≥0.45→squint) **찡그린 ∩눈**, 어두우면 **꾸벅꾸벅**
   (고개 사이클 + 눈꺼풀 동조). 입은 처진 곡선(웃음 금지), 하품은 동그란 O.

### 디자인 규칙 (2026-08-09 현행)
- 실루엣: **반원 돔**(arcToPoint) + 수직 접선으로 이어지는 매끄러운 옆선 +
  살짝 퍼지는 밑단 + 물결 스캘럽 4개. 팔은 **작은 미튼 손**.
- 셰이딩: 흰→라벤더 수직 그라데이션, 좌하단 그늘, 정수리 하이라이트, 볼터치.
...
3. 체크리스트 — `LampRow`: 좌 텍스트(완료 시 삭선), 우 벽 로커 스위치
   (ON 인디케이터는 3겹 발광). 스위치=토글, 행 탭=편집 시트, 롱프레스=메뉴.
   취침 중 스위치 ON = **깨우기(undo)**.
...
- **롤오버**: dayStartHour(기본 06시) 전까지는 어제의 방. 롤오버 시 finalT 기록,
  반복 전개, 지난주 청구서 생성.
...
- 입력 시트: 엔터/Add = 저장 후 연속 입력(시트 유지), 편집 모드는 저장 후 닫힘.
  취침 후 기본 날짜는 내일.
...
2. `flutter test` — 58개 전부 통과가 기준선. UI 변경 시 위젯 테스트가 히트
