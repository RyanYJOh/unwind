  ...
  testWidgets('sleepiness 스크럽 0→1과 이벤트 발사에 크래시가 없다', (tester) async {
    ...
    await pump(
        ...
        const GhostPainterView(
            sleepiness: 0.5, event: GhostEvent.checkOff, eventTick: 1));
    ...
    await pump(
        tester,
        const GhostPainterView(
            sleepiness: 0.5, event: GhostEvent.wakeUpHappy, eventTick: 2));
    ...
    await pump(
        tester,
        const GhostPainterView(
            sleepiness: 1.0, event: GhostEvent.allDone, eventTick: 3));
