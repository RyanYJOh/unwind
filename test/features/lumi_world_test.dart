...
/// 세계관 (2026-08-15): Lumi의 취침시간·기상시간과 다크서클.
...
void main() {
  ...
  Future<LumiModeState> settledMode(ProviderContainer c) async {
    final sub = c.listen(lumiModeProvider, (_, _) {});
    await c.read(settingsControllerProvider.future);
    await pumpEventQueue();
    final mode = c.read(lumiModeProvider);
    sub.close();
    return mode;
  }

  Future<LumiModeState> modeWithPendingTodoAt(int hour) async {
  ...
  group('취침시간 경계 (기본 22시)', () {
    test('21시: 아직 낮의 일과 — 미완 항목이 있어도 행복하다', () async {
      ...
      expect(mode.mode, LumiMode.day);
    });

    test('22시: 취침시간인데 불이 남았다 — 못 자는 밤', () async {
      final mode = await modeWithPendingTodoAt(22);
      expect(mode.mode, LumiMode.nightAwake);
      expect(mode.dazzle, greaterThan(0));
    });

    test('22시의 빈 방 — 만족스러운 잠', () async {
      final c = makeContainer(todayAt(22));
      expect((await settledMode(c)).mode, LumiMode.asleep);
    });
  });

  group('온보딩 시간 매핑 (2026-08-15)', () {
    test('Lumi는 3시간 먼저 자고 1시간 먼저 일어난다 — 자정 넘김 포함', () {
      expect(lumiBedtimeFrom(23), 20);
      expect(lumiBedtimeFrom(0), 21); // 자정 취침 → 21시
      expect(lumiBedtimeFrom(2), 23);
      expect(lumiWakeFrom(7), 6);
      expect(lumiWakeFrom(4), 3);
      expect(lumiWakeFrom(0), 23);
    });
  });
